...
function useMarketData(
  ...
  const [data, setData] = useState<MarketData>({
    goldPrice: initialGoldPrice === null ? null : formatCurrency(roundToTwo(initialGoldPrice)),
  ...
  }, [initialGoldPrice, initialTenYearYield]);
...
// value tracking. Themed to the silver accent and transparent so it sits on
...
function TradingViewSymbolOverview() {
  ...
  useEffect(() => {
    ...
    script.innerHTML = JSON.stringify({
      symbols: [["Silver", `${SILVER_SYMBOL}|1D`]],
      chartOnly: false,
      width: "100%",
      height: "100%",
      locale: "en",
    ...
    >
      ...
      <div className="price-surface-content">
        <div className="flex items-start justify-between gap-4">
          <div>
            <p className="text-xl sm:text-3xl font-semibold tracking-tight text-slate-50">Silver Price</p>
          </div>
          <button className="price-widget-chip rounded-xl border px-3 py-2 text-sm font-medium text-slate-300">1D</button>
        </div>
...
export default function SilverPriceTodayPage({
  ...
  const marketData = useMarketData(initialGoldPrice, initialTenYearYield);
  ...
  return (
    <div className="min-h-screen bg-[#020203] text-slate-100 overflow-x-hidden">
      ...
      <div className="relative z-10 mx-auto max-w-6xl px-4 pb-20 pt-10 lg:px-8">
        ...
        >
          ...
          <div className="price-surface-content grid gap-8 lg:grid-cols-[1.05fr_0.95fr]">
            <div>
              ...
              <PriceChangeStats figures={priceChanges} assetLabel="silver" />
            </div>
        ...
        <PricePageFooterNote asset="silver" />
