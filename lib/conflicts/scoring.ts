import type { SeverityLabel } from "./schema";

// High-severity conflict indicators — any match gives a significant boost
const HIGH_SEVERITY_TERMS =
  /\b(missile|airstrike|air strike|drone strike|shelling|artillery|bombing|explosion|incursion|ambush|mass casualty|cross-border strike|rocket fire|offensive|bombardment|barrage|mortar|grenade|raid)\b/i;
...
const MODERATE_KEYWORDS =
  /\b(clashes?|clash|troops|militia|armed|forces|soldiers|siege|capture|frontline|battle)\b/i;

// Dampening — de-escalation or non-violent news signals
const DEESCALATION_KEYWORDS =
  /\b(ceasefire|talks?|negotiation|negotiations|truce|diplomacy|diplomatic|summit|agreement|statement|delegation|discussion)\b/i;

// Hotspot-level severe override: ensures at least Medium when found in headlines
...
export const CATEGORY_DEFINITIONS = [
  {
    id: "airstrikes",
    label: "Airstrikes",
    keywords: ["airstrike", "air strike", "artillery", "shelling", "bombardment", "bombing"],
  },
  {
    id: "ground-clashes",
    label: "Ground clashes",
    keywords: ["clash", "clashes", "battlefield", "incursion", "insurgent", "militia", "troops"],
  },
  {
    id: "explosions",
    label: "Explosions",
...
export function getRecencyBoost(input: {
  ...
  return Math.round(clamp(1 - ageMs / RECENCY_WINDOW_MS, 0, 1) * (input.maxBoost ?? 20));
}
...
export function scoreSeverity(input: {
  ...
  gdeltTone?: number;
  now?: Date;
}): { severityScore: number; severityLabel: SeverityLabel; severityReasons: SeverityReasons } {
  const now = input.now ?? new Date();
  ...
  if (recencyBoost > 0) {
    score += recencyBoost;
    if (recencyBoost >= 15) reasons.push("Recent event boost applied");
  }

  if (typeof input.gdeltTone === "number" && input.gdeltTone < 0) {
    const toneBoost = clamp(Math.abs(input.gdeltTone) / 8, 0, 15);
    score += toneBoost;
    if (toneBoost >= 8) reasons.push("Negative tone increased risk score");
  }

  const severityScore = Math.round(clamp(score, 0, 100));
  return { severityScore, severityLabel: severityLabelFromScore(severityScore), severityReasons: reasons };
}
...
export function scoreHotspotSeverity(input: {
  ...
  const maxHotspotCount =
    typeof input.maxHotspotCount === "number" && Number.isFinite(input.maxHotspotCount)
      ? input.maxHotspotCount
      : 0;
  const denominator = Math.log1p(maxHotspotCount > 1 ? maxHotspotCount : HOTSPOT_BASELINE_MAX);
  ...
  if (typeof input.gdeltTone === "number" && input.gdeltTone < 0) {
    ...
    score += toneBoost;
    if (toneBoost >= 8) reasons.push("Negative tone boost applied");
  }
  ...
  if (input.headlineText) {
    if (HOTSPOT_OVERRIDE_TERMS.test(input.headlineText)) {
      ...
      reasons.push(`Headline contains severe term: ${match ?? "conflict indicator"}`);
      score = hotspotCount >= 3 ? Math.max(score, 40) : Math.max(score, 34);
      score += 12;
