...
export function scoreSeverity(input: {
  ...
  gdeltTone?: number;
  now?: Date;
}): { severityScore: number; severityLabel: SeverityLabel } {
  const now = input.now ?? new Date();
  const title = input.title.toLowerCase();
  let score = 0;

  if (STRONG_KEYWORDS.test(title)) score += 25;
  if (MODERATE_KEYWORDS.test(title)) score += 15;
  if (DEESCALATION_KEYWORDS.test(title)) score -= 10;

  score += getRecencyBoost({ dateIso: input.dateIso, now, maxBoost: 20 });
  ...
  return { severityScore, severityLabel: severityLabelFromScore(severityScore) };
}
