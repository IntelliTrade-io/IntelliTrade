...
- [ ] **Visual-verify the per-pair lot size pages + the pre-selected calculator** (2026-07-18, branch `feat/per-pair-calc-seo`). New static SEO routes `/lotsizecalculator/[pair]` (33 symbols, e.g. `/lotsizecalculator/eurusd`, `/xauusd`, `/gbpjpy`) pre-select the pair via a new `initialPair` prop on the golden `LotSizeCalculator`. Build is green (308 static pages) and the math is unit-tested, but the golden calculator UI changed (pair now seeded from the route) — per the standing rule, eyeball it on `:3000`: (1) a per-pair page loads with the right pair already chosen; (2) a returning visitor with saved inputs still gets balance/risk restored but the URL pair wins; (3) the main `/lotsizecalculator` still defaults to EURUSD and the new "Lot size by pair" chip grid links resolve. If good, tell Claude "per-pair pages verified".
