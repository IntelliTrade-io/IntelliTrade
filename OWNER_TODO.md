...
- [ ] **Vercel Attack Challenge Mode is ON** (every non-browser request to intellitrade.tech gets 429 + JS challenge, `X-Vercel-Mitigated: challenge`). Verified bots (Googlebot) are exempt per Vercel docs, so SEO/AdSense crawling should be fine — but confirm this was enabled deliberately and reconsider: Vercel intends it for active attacks; permanently on it blocks link-preview bots, uptime monitors and SEO tools. Bot Protection (managed ruleset) is the always-on alternative.
