...
- [x] ~~**Visual-verify the per-pair lot size pages + the pre-selected calculator**~~ **VERIFIED 2026-07-18** by owner on `:3000` (branch `feat/per-pair-calc-seo`). Static SEO routes `/lotsizecalculator/[pair]` (33 symbols) pre-select the pair via the new `initialPair` prop on the golden `LotSizeCalculator`; per-pair pre-selection, saved-input restore, and the hub "Lot size by pair" grid all confirmed good. Golden calc change cleared for merge.
