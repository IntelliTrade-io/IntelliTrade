...
- [ ] **Cofounder: add 3 fields to the `marketContext` Studio schema** — snippet + content guidance in **`SANITY_SCHEMA_UPGRADE.md`** (repo root). The site already renders `stats` / `weekRecap` / `relatedLinks` when present (verified live on the gold doc, which now carries two demo related-links he can edit). Until the Studio schema is updated he can't edit the new fields in the Studio UI.
