...
- [ ] **6 non-USD crosses fail to fetch** (GBPAUD, AUDJPY, AUDCAD, NZDCAD, CHFJPY, CADCHF → `1hour: Terminal: Call failed`). Not a refactor regression — MetaQuotes-Demo doesn't have these subscribed/selected. Fix: MT5 → Market Watch → right-click → **Show All** (or Symbols dialog → enable those 6), then re-run a scan to confirm 28/28. Low priority; the 22 majors write fine.
