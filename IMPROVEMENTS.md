...
- ~~**Per-pair calculator SEO pages**~~ *(all three pair-based calcs done 2026-07-18)* — shipped `/lotsizecalculator/[pair]`, `/pipvaluecalculator/[pair]` **and** `/margincalculator/[pair]` for all 33 `composePairsFrom` symbols (majors both ways, popular crosses, XAU/XAG/BTC/ETH). Static (`generateStaticParams` + `dynamicParams=false`, 404s unknown slugs), each self-canonical (targets its own long-tail query, complements the hub — not cannibalize), with per-symbol H1/meta, per-pair facts/values, pair-specific FAQ+FAQPage schema, SoftwareApplication + BreadcrumbList schema, a per-symbol `next/og` card, sitemap entries, sibling-pair cross-links, and a "by pair" chip grid on each hub. Lot-size pages carry an exact quote-currency worked example; pip-value pages carry the standard/mini/micro per-lot table (from `pipValueLots`); margin pages carry a leverage→margin-% reference (1/leverage, pair-independent, with a broker-cap caveat on metals/crypto — no fabricated exact margin since that needs a live price). All three cross-link to each other's matching pair page. Pre-selection uses a new `initialPair` prop on `LotSizeCalculator`, `PipValueCalculator` and `MarginCalculator` (on lot size the URL pair wins over persisted state; all other saved inputs restore; pip/margin have no persistence). Pure descriptors + example/value math in `lib/pair-meta.ts` (tested, 17 cases). Compounding + forex-market-hours have no pair dimension → deliberately excluded. NOTE: golden lot-size calc change owner-verified 2026-07-18.
- ~~**Risk as fixed money amount**~~ *(done 2026-07-18)* — %/account-currency toggle beside "Risk per trade" on the lot size calculator; money mode converts to the equivalent percentage in the component so `computePositionSize` is untouched. Persistence gained `riskMode`/`riskAmount` without a version bump (legacy payloads restore in percent mode); applying a Pro template switches back to percent. Owner visual verify pending (golden calc).
...
- ~~**`/blog/all` pagination is client-only**~~ *(done 2026-07-18)* · server-paginated `?page=N` with plain link anchors (rel prev/next), per-page canonical/title, `?page=1`/garbage redirecting to the bare URL and past-the-end 404ing. Client component now only renders the slice it is given.
- ~~**Blog tags are raw slugs end-to-end**~~ *(done 2026-07-18 — premise outdated)* · tags in Sanity are now plain English ("EUR/USD outlook", 17 distinct / 75 tagged posts), so no display map was needed. Shipped `slugifyTag` + `collectTagCounts` in `lib/blog.ts` (tested) and programmatic `/blog/tag/<slug>` ISR pages (per-tag metadata + CollectionPage/ItemList schema, 404 unknown, dynamicParams for new pipeline tags), with crawl paths: "Browse by topic" chips on `/blog/all`, post-page tag badge links, sitemap entries.
