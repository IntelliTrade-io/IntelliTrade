...
- ~~**Per-pair calculator SEO pages**~~ *(lot-size done 2026-07-18)* — shipped `/lotsizecalculator/[pair]` for all 33 `composePairsFrom` symbols (majors both ways, popular crosses, XAU/XAG/BTC/ETH). Static (`generateStaticParams` + `dynamicParams=false`, 404s unknown slugs), each self-canonical (targets its own long-tail query, complements the hub — not cannibalize), with per-symbol H1/meta, contract-spec facts, an exact quote-currency worked example, pair-specific FAQ+FAQPage schema, SoftwareApplication + BreadcrumbList schema, a per-symbol `next/og` card, sitemap entries, sibling-pair cross-links, and a "Lot size by pair" chip grid on the hub. Pre-selection uses a new `initialPair` prop on `LotSizeCalculator` (URL pair wins over persisted; all other saved inputs restore). Pure descriptors + example math in `lib/pair-meta.ts` (tested). **Pip-value equivalent still open** — replicate the same pattern on `/pipvaluecalculator/[pair]` (reuse `lib/pair-meta.ts`; PipValueCalculator would need the same `initialPair` prop). NOTE: golden calc touched → owner visual-verify queued in `OWNER_TODO.md`.
