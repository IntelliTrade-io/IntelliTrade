...
- ~~**Per-pair calculator SEO pages**~~ *(lot-size + pip-value done 2026-07-18)* — shipped `/lotsizecalculator/[pair]` **and** `/pipvaluecalculator/[pair]` for all 33 `composePairsFrom` symbols (majors both ways, popular crosses, XAU/XAG/BTC/ETH). Static (`generateStaticParams` + `dynamicParams=false`, 404s unknown slugs), each self-canonical (targets its own long-tail query, complements the hub — not cannibalize), with per-symbol H1/meta, per-pair facts/values, pair-specific FAQ+FAQPage schema, SoftwareApplication + BreadcrumbList schema, a per-symbol `next/og` card, sitemap entries, sibling-pair cross-links, and a "by pair" chip grid on each hub. Lot-size pages carry an exact quote-currency worked example; pip-value pages carry the standard/mini/micro per-lot table (from `pipValueLots`) and reciprocally cross-link to the matching lot-size pair page. Pre-selection uses a new `initialPair` prop on both `LotSizeCalculator` and `PipValueCalculator` (on lot size the URL pair wins over persisted state; all other saved inputs restore). Pure descriptors + example/value math in `lib/pair-meta.ts` (tested, 17 cases). NOTE: golden lot-size calc change owner-verified 2026-07-18.
