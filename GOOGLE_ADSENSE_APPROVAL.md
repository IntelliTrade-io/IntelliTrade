...
- [ ] **Text-rich tool pages**: replicate the lot-size-calculator SEO upgrade (explanations, worked examples, FAQ) on every public calculator/price page. *Progress 2026-07-12:* gold-price-today done — FAQ expanded 3→7 (gram/kilo conversion, spot vs futures, dollar relationship, worldwide pricing) + FAQPage JSON-LD added (page had a visual FAQ but no structured data). Remaining: silver, oil, bitcoin (same pattern, one per session).
...
- [x] Crawl for broken links / empty pages / 404 — done 2026-07-12 against a local production build (**live crawl impossible: Vercel Attack Challenge Mode returns 429 to every non-browser client** — see OWNER_TODO; verified bots like Googlebot are exempt per Vercel docs, so SEO unaffected, but owner should confirm the mode is on deliberately). Findings: all 19 public pages 200 with substantive HTML; `/static/favicons/site.webmanifest` was 307→login (middleware gap — **fixed**, `.webmanifest` added to matcher exclusions); unknown URLs 307→login instead of 404 (soft-404; deliberate default-deny middleware — keeping security posture over SEO nicety, low risk since no legacy dead URLs exist).
