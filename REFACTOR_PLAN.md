...
- [ ] Dependency diet (owner 2026-07-05: clean/modern/minimal): swap `supabase-py` (heavy transitive tree — 20+ packages for a handful of PostgREST inserts) for plain `requests` — watchdog.py already proves the pattern; do it across scanners upload + backend `supabase_writer` in one pass, after mocked tests cover the writers (pairs with 6.8). Re-evaluate `lxml`/`feedparser` per-source during the 6.3 split.
- Note: Consolidation done 2026-07-05: both requirements.txt deleted; pyproject extras (`[scraper]`, `[mt5]`, `[dev]`) + `requirements-lock.txt` (fresh py3.14 venv, full pytest validated under the locked pandas 3.0.3/numpy 2.5.1). All 3 workflows install from package + lock on py3.14. Diet item stays open.
