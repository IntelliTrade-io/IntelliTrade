...
- [ ] Delete `app/blog/seo.tsx` (100% commented; Phase 4 leftover).
- [ ] Execute the moves as pure `git mv` commits (no logic change), one feature at a time, import paths updated per batch; build green per batch.
...
- [ ] Lock the final convention into repo `CLAUDE.md` (per §5-conventions-home rule) so it survives the refactor.
- Note:

**5.4 — De-duplicate components**
- [x] `lot-size-calculator.tsx` (v1, unused) — remove ✅ (2026-07-04, zero importers; its `#position-size-calculator-564055` CSS died with it in 5.1); ~~keep `-2`~~ rename `-2` → canonical still pending (do with 5.3 move batch).
