...
- Note (2026-07-08, partial via Task D): Python half landed — `.github/workflows/python-tests.yml` runs `pytest` + coverage floor (50%) on any `**.py` change. **Still open for 7.3:** the frontend job (`npm run build` / `npm test` / `tsc --noEmit` / lint), branch-protection merge gating (owner-only, → OWNER_TODO), and the security regression check (anon key denied on premium tables). Also worth folding in the two existing scheduled scraper/scanner workflows' install path for consistency.
