...
- [x] Hardcoded `C:\IntelliTrade\*` in `scripts/vps/setup_windows_tasks.ps1`, `config_template.env`, `export_eurusd_m15.py`. Move to env/config with sensible defaults so the tree isn't Windows-path-locked.
...
- Note (2026-07-12, **6.6 closed**): resolved via supersession, not by editing the baseline files. The **live** VPS deploy path since 6.7 (executed 2026-07-09) is `scripts/vps/bootstrap.ps1`, which is fully env-parameterized — `-IntelliTradeHome`/`INTELLITRADE_HOME` (Machine scope, inherited by Task Scheduler) + `-RepoDir` + `-PythonExe`, no path-locking; the default `C:\IntelliTrade` is an overridable default, not a hardcode. Python side already parameterized (`intellitrade_scanners/config.py` via `INTELLITRADE_HOME`, `economic_calendar/paths.py` via `ECON_CALENDAR_HOME`); `export_eurusd_m15.py` was found to have no functional hardcode (Task B). The only residual `C:\IntelliTrade` literals live in the **retired** `setup_windows_tasks.ps1` + `config_template.env` — the pre-git baseline, no longer on the live path (bootstrap replaced them). Their physical deletion is owner/VPS-gated behind the rollback window (OWNER_TODO "Retire the old flat files"), so they stay on disk as the rollback baseline but count as superseded for 6.6. **Box checked.**
